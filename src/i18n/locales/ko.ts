import type { Translations } from '../types';

export const ko: Translations = {
  meta: {
    htmlLang: 'ko',
    siteDescription:
      'RPGSkills — 마인크래프트 서버를 RPG 어드벤처로. 커스텀 몬스터, 스킬, 퀘스트, 레이드 등.',
  },
  nav: {
    home: '홈',
    groupBackground: '배경',
    groupDownload: '다운로드',
    groupCommunity: '커뮤니티',
    story: '스토리',
    features: '기능',
    classes: '직업',
    install: '설치',
    download: '다운로드',
    boardFree: '자유게시판',
    boardQna: 'Q&A',
    support: '지원',
    donate: '후원',
    ariaMain: '주 메뉴',
  },
  community: {
    firebaseNotConfigured:
      'Firebase가 설정되지 않았습니다. 프로젝트 루트에 .env 파일을 만들고 VITE_FIREBASE_* 값을 채운 뒤 다시 빌드하세요.',
    authLoading: '로그인 상태 확인 중…',
    signInHint: '글 작성·댓글은 Google 계정으로 로그인한 뒤 이용할 수 있습니다.',
    signInGoogle: 'Google로 로그인',
    signOut: '로그아웃',
    todayVisitors: '오늘 방문',
    loading: '불러오는 중…',
    errorGeneric: '요청을 처리하지 못했습니다. 잠시 후 다시 시도해 주세요.',
    free: {
      pageTitle: '자유게시판',
      pageDescription: '플러그인·서버·플레이 이야기를 자유롭게 나눠 보세요.',
      newPost: '새 글',
      empty: '아직 글이 없습니다. 첫 글을 남겨 보세요!',
      titlePlaceholder: '제목',
      bodyPlaceholder: '내용',
      submit: '등록',
      back: '목록',
      comments: '댓글',
      commentPlaceholder: '댓글을 입력하세요',
      addComment: '댓글 등록',
      deletePost: '글 삭제',
      deleteComment: '댓글 삭제',
      solved: '해결됨',
      markSolved: '해결됨으로 표시',
      markOpen: '미해결로 표시',
      solvedBadge: '해결',
    },
    qna: {
      pageTitle: 'Q&A',
      pageDescription: '설치·오류·설정 관련 질문과 답변을 남겨 주세요.',
      newPost: '질문 등록',
      empty: '아직 질문이 없습니다.',
      titlePlaceholder: '질문 제목',
      bodyPlaceholder: '증상, 버전, 재현 방법 등을 적어 주세요',
      submit: '질문 등록',
      back: '목록',
      comments: '답변',
      commentPlaceholder: '답변을 입력하세요',
      addComment: '답변 등록',
      deletePost: '질문 삭제',
      deleteComment: '답변 삭제',
      solved: '해결됨',
      markSolved: '해결됨으로 표시',
      markOpen: '미해결로 표시',
      solvedBadge: '해결',
    },
  },
  lang: {
    label: '언어',
    en: 'English',
    ko: '한국어',
  },
  hero: {
    tagline: '위더의 그림자 아래, 서버를 RPG 어드벤처로 바꿔 보세요',
    intro:
      '커스텀 몬스터, 무기·방어구, 스킬, 퀘스트, 보스 레이드, 파티 시스템으로 서버를 RPG 어드벤처로 바꿔 보세요.',
    download: '다운로드',
    spigot: 'Spigot 페이지',
    gettingStarted: '시작하기',
    compatible: '호환 버전',
    mcVersionLabel: 'Spigot/Paper API 26.1.2 (26.1.2-R0.1-SNAPSHOT) · Minecraft 26.1.2',
    battleImageAlt:
      '위더가 보랏빛 안개 속에 강림하고, 검사·궁수·마법사 등 영웅들이 맞서 돌진하는 장면',
  },
  home: {
    whyTitle: 'RPGSkills를 선택하는 이유',
    viewAllFeatures: '전체 기능 보기',
    viewClasses: '직업·조합식 가이드',
    championsTitle: '32명의 영웅이 맞선다',
    championsHint:
      '각 직업은 고유한 무기와 스킬로 전장에 서 있습니다. 위더의 그림자가 드리운 세계에서, 당신이 선택한 길이 마지막 방어선이 됩니다.',
  },
  story: {
    pageTitle: '세계관 — 위더의 귀환',
    pageDescription:
      '봉인이 풀린 위더, 잠식되는 대지, 그리고 32개 직업으로 일어선 모험가들의 이야기.',
    epigraph:
      '「고대의 봉인이 부서진 날, 하늘은 보랏빛으로 갈라졌고, 대지는 다시 위더의 숨결을 기억하기 시작했다.」',
    teaserLabel: 'RPGSkills 세계관',
    teaserTitle: '봉인이 풀리고, 세계가 위더의 숨결에 잠기다',
    teaserParagraphs: [
      '수백 년간 잠들어 있던 위더의 봉인이 무너지며, 보랏빛 안개가 대륙을 덮었습니다. 몬스터는 날뛰고, 약탈자는 마을을 짓밟으며, 평범한 모험가들마저 서서히 위더의 기운에 잠식되기 시작했습니다.',
      '살아남기 위해 각자 직업을 선택하고 강해진 영웅들이 모였습니다. 최종 목표는 단 하나—이 세계의 위더를 쓰러뜨리고, 평행 차원 너머로 번져 가는 재앙의 뿌리까지 끊는 것입니다.',
    ],
    readFull: '전체 스토리 읽기',
    chapters: [
      {
        title: '제1장 — 깨진 봉인',
        paragraphs: [
          '전설에 따르면, 최초의 위더는 세계의 균형을 삼킨 뒤 현자들의 희생으로 봉인되었다고 전해집니다. 그러나 시간이 그 봉인을 갉아 먹었고, 차원의 균열이 벌어지자마자 하늘은 다시 보랏빛으로 물들었습니다.',
          '봉인이 완전히 부서진 순간, 대륙 곳곳에 위더의 잔영이 떠올랐습니다. 엔더의 심연, 네더의 불길, 정글의 어둠까지—어디서든 같은 공포가 되살아났습니다. 사람들은 처음엔 단순한 재앙으로 여겼지만, 곧 이것이 끝이 아닌 시작임을 깨달았습니다.',
        ],
      },
      {
        title: '제2장 — 잠식되는 대지',
        paragraphs: [
          '위더의 기운은 공기 속에 스며들었습니다. 밤이면 보랏빛 안개가 마을을 감싸고, 낮이면 몬스터들이 예전보다 거칠고 교활해졌습니다. 약탈자 무리는 실패한 습격을 두려워하지 않았고, 주민들은 문을 닫은 채 하루하루를 버텼습니다.',
          '모험가—서버에 접속한 당신을 포함한 모든 이—역시 예외가 아니었습니다. 오래 싸울수록 몸속 깊이 차가운 피로가 맴돌고, 스킬을 익힐수록 위더의 그림자가 시야 끝에서 웃는 듯했습니다. 그러나 잠식은 동시에 각성이기도 했습니다. 절망 속에서 사람들은 처음으로 직업을 선택하고, 무기를 들고, 서로의 등을 맡기기 시작했습니다.',
        ],
      },
      {
        title: '제3장 — 직업을 택한 영웅들',
        paragraphs: [
          '검을 든 검사, 활을 당긴 궁수, 연금술의 불꽃을 다루는 화학자, 바다의 기운을 끌어오는 오션 나이트—서른두 갈래의 길이 열렸고, 각 길은 서로 다른 방식으로 위더의 군세를 갈랐습니다.',
          '파티는 마을을 지키는 방벽이 되었고, 퀘스트는 잃어버린 희망을 되찾는 나침반이 되었습니다. 필드에서 쓰러뜨린 위더는 레이드의 문을 열었고, 그 문 너머에서는 더 거대한 시험이 기다렸습니다. 영웅들은 이제 단순히 살아남는 것이 아니라, 세계가 다시 숨 쉬게 만드는 법을 배우고 있었습니다.',
        ],
      },
      {
        title: '제4장 — 평행 차원 너머',
        paragraphs: [
          '첫 번째 위더가 쓰러졌을 때, 하늘은 잠시 맑아진 듯했습니다. 그러나 차원의 균열은 닫히지 않았습니다. 평행한 세계들—다른 시공에 존재하는 또 다른 대륙들—에서도 봉인이 함께 무너졌고, 그곳의 위더와 몬스터들이 서로의 재앙을 키우고 있었습니다.',
          '진정한 종언은 하나의 보스를 처치하는 데 그치지 않습니다. 오버월드·네더·엔드, 그리고 위더 레이드의 심장까지—모든 차원의 균열을 메우고, 평행 세계에 남은 위더의 잔재를 끝까지 추적해야 합니다. 당신이 고른 직업, 당신이 쌓은 숙련, 당신이 이끄는 파티가 마지막 봉인이 될지도 모릅니다.',
        ],
      },
    ],
    ctaTitle: '당신의 이야기를 이어가세요',
    ctaBody:
      '직업을 선택하고, 스킬을 익히고, 레이드에 도전하세요. 세계는 아직 끝나지 않았고, 영웅의 자리는 비어 있습니다.',
    ctaClasses: '직업 살펴보기',
    ctaDownload: '플러그인 다운로드',
  },
  features: {
    pageTitle: '기능',
    pageDescription: '몬스터, 아이템, 퀘스트, 레이드, 파티, 설정 등 RPGSkills 핵심 시스템 안내.',
    sections: [
      {
        title: '커스텀 몬스터 & 전투',
        items: [
          '특수 능력을 가진 유니크 몬스터',
          '바이옴별 몬스터 스케일링 — 바이옴에 따라 강도 변화',
          '콤보: 5스택 시 추가 스폰, 마일스톤마다 필드 보스',
        ],
      },
      {
        title: '무기·방어구·스킬',
        items: [
          '다양한 커스텀 무기·방어구',
          '8계열 × 4역할 32직업 — 직업 페이지 참고',
          '스킬·숙련도·피해량: /rpg skill, /rpg rank, /rpg graph',
          '리소스팩 지원 (Beta)',
        ],
      },
      {
        title: '바이옴 속성 & 무기 강화',
        items: [
          '바이옴마다 고유 속성 조각으로 무기 강화',
          '순서: Earth(평원) → Wind(산/언덕) → Frost → Ocean → Dark(숲) → Hyper(사막) → Burning(악지) → Poison(늪) → Wild(정글)',
          '네더라이트 무기 레벨업; 삼지창·철퇴는 네더라이트 없이 직접 강화 가능',
          '/smith 로 장비 제작·네더라이트급 무기 획득',
        ],
      },
      {
        title: '퀘스트 & 보스 레이드',
        items: [
          '구조물 전리품 후 퀘스트 NPC 등장',
          '정글·네더: 몬스터 처치 시 포탈 생성 → 우클릭으로 레이드',
          '엔드: EnderMimic 처치 시 레이드 포탈',
          '위더 레이드 (Beta): 필드 위더 처치 후 포탈',
          '재설치 시 폴더 삭제: OverworldRaid, NethercoreRaid, EndercoreRaid, WitherRaid',
        ],
      },
      {
        title: '파티 & 서버 설정',
        items: [
          '파티: create, join, password, invite, kick, leave, assemble, list',
          'config 다국어 지원',
          '월드별 플러그인 on/off',
          'plugins/에 JAR 배치 후 config·리소스팩 적용',
        ],
      },
    ],
  },
  install: {
    pageTitle: '설치',
    pageDescription: '플러그인 설치, 버전 호환, 리소스팩 적용 가이드.',
    versionNotice: '버전',
    steps: [
      {
        title: '1. 서버 준비',
        body: 'Paper 또는 Spigot Minecraft 26.1.2 + Spigot API 26.1.2 이상 (pom.xml spigot-api 26.1.2-R0.1-SNAPSHOT 기준).',
      },
      {
        title: '2. 플러그인 설치',
        body: 'RPGskills.jar를 plugins 폴더에 넣고 서버를 재시작합니다. config는 plugins/RPGskills/ 에 생성됩니다.',
      },
      {
        title: '3. 리소스팩 적용',
        body:
          'Download 페이지에서 리소스팩을 받습니다. GitHub Code → Download ZIP 또는 Google Drive를 사용하세요. ' +
          'server.properties resource-pack 또는 클라이언트 개별 적용이 가능합니다.',
      },
      {
        title: '4. 월드·설정',
        body:
          'config에서 언어·월드별 활성화를 설정합니다. 레이드 재설치 시 raid 월드 폴더 삭제를 권장합니다.',
      },
    ],
  },
  download: {
    pageTitle: '다운로드',
    pageDescription:
      '최신 플러그인 JAR와 리소스팩을 받으세요. JAR 링크는 GitHub Releases API로 자동 갱신됩니다.',
    pluginJar: '플러그인 JAR',
    latestFromGithub: 'GitHub 최신 릴리스',
    loading: ' — 불러오는 중…',
    apiFallback: 'API 대체',
    downloadJar: 'JAR 다운로드',
    allReleases: '전체 릴리스',
    resourcePackGithub: '리소스팩 (GitHub)',
    resourcePackGithubMeta: 'RpgSkills 리포지토리 — Code → Download ZIP',
    downloadZip: 'ZIP 다운로드',
    repository: '리포지토리',
    noGithubReleaseHint:
      'GitHub Releases에서 최신 JAR를 불러오지 못했습니다. chw3021/rpgskills-releases 전체 릴리스 또는 아래 Spigot을 이용하세요.',
    getJarFromSpigot: 'Spigot에서 받기',
  },
  support: {
    pageTitle: '지원',
    pageDescription: '커맨드 참고, FAQ, 버그 제보 안내.',
    reportTitle: '버그 제보',
    reportIntro: '아래 정보를 포함하면 원인 파악이 빨라집니다.',
    faqTitle: '자주 묻는 질문',
    commandsTitle: '커맨드',
    playerCommands: '플레이어',
    partyCommands: '파티',
    adminCommands: '관리자',
    commandCol: '커맨드',
    descriptionCol: '설명',
    reportChannels: [
      {
        label: 'GitHub Issues',
        description: '버그·기능 제안 (로그·버전 정보 포함)',
      },
      {
        label: 'Spigot 리소스',
        description: '리뷰·토론·다운로드',
      },
    ],
    bugReportChecklist: [
      '서버 소프트웨어 및 버전 (Paper/Spigot, Minecraft 26.1.2)',
      'RPGSkills JAR 버전 또는 Releases 태그',
      '설치된 다른 플러그인 목록 (/plugins)',
      '재현 단계',
      '콘솔 전체 스택 트레이스',
      '리소스팩 적용 여부',
      '관련 월드·바이옴',
    ],
  },
  donate: {
    pageTitle: '후원',
    pageDescription: 'RPGSkills 개발을 응원해 주세요. 후원은 기능 해금과 무관합니다.',
    intro: 'RPGSkills는 무료 플러그인입니다. 후원은 개발·서버 비용·업데이트에 도움이 됩니다. ',
    introBold: '후원은 프리미엄 기능 해금과 연동되지 않습니다.',
    paypalButton: 'PayPal로 후원하기',
    disclaimer1: '· 후원은 자발적인 지원이며, 디지털 콘텐츠 환불이 어려울 수 있습니다.',
    disclaimer2: '· 결제·영수증 문의는 PayPal 거래 내역을 기준으로 합니다.',
    disclaimer3:
      '· 향후 프리미엄 라이선스는 별도 구매·키 발급 절차로 제공될 예정입니다 (Firebase 연동 예정).',
    premiumTitle: '추후 제공 예정:',
    premiumBody:
      'Firebase 기반 프리미엄 라이선스 구매 및 키 활성화 — PremiumSection 확장을 위한 구조가 준비되어 있습니다.',
  },
  footer: {
    spigot: 'Spigot',
    releases: '릴리스',
    website: '웹사이트',
  },
  commands: {
    player: [
      { cmd: '/rpg dam (d, damage)', desc: '현재 공격력 표시' },
      { cmd: '/rpg element (el)', desc: '속성력·저항 표시' },
      { cmd: '/rpg skill (s)', desc: '스킬 GUI 열기' },
      { cmd: '/rpg escape (es)', desc: '현재 전투 종료' },
      { cmd: '/rpg class (c)', desc: '직업 GUI 열기' },
      { cmd: '/rpg rank (r)', desc: '직업 숙련도 Top 10' },
      { cmd: '/rpg graph (g)', desc: '적에게 가한 피해량' },
      { cmd: '/rpg gclear (gc)', desc: '기록된 피해량 초기화' },
      { cmd: '/smith', desc: '1번 슬롯 장비를 2번 슬롯 재료로 제작' },
    ],
    party: [
      { cmd: '/party create <partyname>', desc: '파티 생성 (방장 됨)' },
      { cmd: '/party join <partyname>', desc: '파티 참가' },
      { cmd: '/party password <password>', desc: '비밀번호로 잠금' },
      { cmd: '/party invite <player>', desc: '플레이어 초대 (방장)' },
      { cmd: '/party kick <player>', desc: '추방 (방장)' },
      { cmd: '/party leave', desc: '파티 탈퇴' },
      { cmd: '/party assemble', desc: '멤버를 방장 위치로 (방장)' },
      { cmd: '/party list', desc: '파티 목록' },
    ],
    admin: [
      { cmd: '/rpg exp [amount]', desc: '경험치 지급' },
      { cmd: '/rpg pro [amount]', desc: '숙련도 포인트 지급' },
      { cmd: '/rpg enchant [enchant] [level]', desc: '주 손 아이템 인챈트' },
      { cmd: '/rpg enchant clear', desc: '주 손 인챈트 제거' },
      { cmd: '/rpg elements (elm)', desc: '속성 인벤토리 열기' },
    ],
  },
  classes: {
    pageTitle: '직업 & 조합식',
    pageDescription: '게임 내 /rpg class GUI와 동일한 32직업 설명 및 커스텀 무기 조합식.',
    tabListLabel: '직업 페이지 섹션',
    tabClasses: '직업',
    tabCrafting: '조합식',
    archetypesTitle: '직업 계열 (GUI 열)',
    rolesTitle: '역할군 (GUI 행)',
    rosterTitle: '전체 직업',
    rosterHint: '스탯은 게임 내 직업 선택 GUI와 동일합니다 (공격·방어·제어·지원·범위·사거리·기동).',
    filterArchetype: '계열',
    filterRole: '역할',
    filterAll: '전체',
    showingCount: '{count}개 직업 표시',
    viewDetail: '상세 보기',
    equipmentLabel: '추천 장비',
    portraitZoom: '{name} 초상화 크게 보기',
    closeLightbox: '이미지 닫기',
    archetypeLabels: {
      warrior: '전사',
      shooter: '사수',
      fighter: '격투가',
      mage: '마법사',
      technician: '전문가',
      marine: '해상',
      avenger: '복수자',
      battleLine: '군진',
    },
    roleLabels: {
      assault: '돌격',
      nuker: '공격',
      suppressor: '제압',
      supporter: '지원',
    },
    difficulty: { easy: '쉬움', normal: '보통', hard: '어려움' },
    stats: {
      attack: '공격',
      defence: '방어',
      control: '제어',
      support: '지원',
      area: '범위',
      range: '사거리',
      mobility: '기동',
    },
  },
  classDetail: {
    pageTitle: '직업 상세',
    notFoundTitle: '직업을 찾을 수 없습니다',
    notFoundBody: '아직 상세 페이지가 준비되지 않았거나 잘못된 주소입니다.',
    backToRoster: '← 직업 목록',
    classIndex: '{n} / {total}',
    storyTitle: '배경',
    skillsTitle: '스킬',
    skillsHint: '게임 내 /rpg skill GUI와 동일한 구성입니다. 피해량은 스킬 레벨·숙련도에 따라 달라집니다.',
    elementLabel: '속성',
    elementEarth: '대지',
    elementWind: '바람',
    elementFire: '화염',
    elementPoison: '독',
    elementLightning: '번개',
    elementWater: '물',
    elementDark: '어둠',
    elementFrost: '서리',
    chainHint: '같은 열 = 연계기 (게임 내 스킬 GUI와 동일한 열)',
    sectionBase: '기본 스킬',
    sectionLimit1: '1차 한계 돌파',
    sectionLimit2: '2차 한계 돌파',
    tierBase: '기본',
    tierLimit1: '1차',
    tierLimit2: '2차',
    proficiencyHint: '숙련도 {exp1} 이상 → 1차 한계 돌파, {exp2} 이상 → 2차 한계 돌파',
    unlockAt: '숙련도 {exp} 이상 시 해금',
    inputLabel: '입력',
    followUp: '연계',
    passive: '패시브',
    ultimate: '궁극기',
    masterLevel: '마스터 LV.{lv}',
  },
  crafting: {
    title: '커스텀 무기 조합',
    intro:
      '너클·완드·단검은 대체 재료로 제작합니다. 활·쇠뇌·낚싯대는 나무 대신 다른 재료를 넣을 수 있습니다.',
    smithTitle: '/smith:',
    smithBody: '첫 슬롯 장비를 둔 채 두 번째 슬롯 재료로 제련합니다. 네더라이트급 무기도 획득 가능합니다.',
    alternateTools: '활·쇠뇌·낚싯대: 제작 시 나무 부분을 다른 재료로 대체.',
    mirrorNote: '모든 조합식은 좌우 반전(거울)이 가능합니다.',
    legendMaterial: '재료 (x)',
    legendStick: '막대 (a)',
    legendMaterialShort: '재',
    legendStickShort: '막',
    recipeNames: { knuckle: '너클', wand: '완드', dagger: '단검' },
  },
  faq: [
    {
      id: 'version-mismatch',
      question: '서버 버전이 맞지 않으면 어떻게 되나요?',
      answer:
        'RPGSkills는 Spigot/Paper Minecraft 26.1.2 (API 26.1.2) 기준으로 개발·테스트됩니다. ' +
        '다른 메이저 버전에서는 명령어·몬스터·GUI가 동작하지 않거나 콘솔 오류가 발생할 수 있습니다.',
    },
    {
      id: 'resource-pack',
      question: '리소스팩을 적용하지 않으면 텍스처가 깨져 보여요',
      answer:
        '커스텀 무기·방어구·몬스터는 리소스팩(Beta)이 필요합니다. Download 페이지에서 ' +
        'GitHub ZIP 또는 Google Drive 팩을 받아 서버 resource-pack 또는 클라이언트에 적용하세요.',
    },
    {
      id: 'party-error',
      question: '/party 명령이 오류를 냅니다',
      answer:
        '다른 플러그인과 명령 충돌, 구버전 JAR, 손상된 설정이 원인일 수 있습니다. ' +
        '전체 콘솔 로그, /plugins 목록, RPGSkills 버전을 Issues에 첨부해 주세요.',
    },
    {
      id: 'raid-world',
      question: '보스 레이드 포탈·월드가 이상해요',
      answer:
        '플러그인 재설치 후 서버 폴더의 OverworldRaid, NethercoreRaid, ' +
        'EndercoreRaid, WitherRaid 폴더 삭제를 권장합니다.',
    },
    {
      id: 'monster-lag',
      question: '몬스터가 너무 많이 스폰되어 렉이 납니다',
      answer:
        '바이옴별 스케일링과 콤보 시스템으로 추가 스폰이 발생합니다. config에서 월드별 설정을 확인하고, ' +
        '재현 조건(바이옴, 콤보 수)을 제보에 포함해 주세요.',
    },
    {
      id: 'backpack-gui',
      question: '전리품·가방 GUI 버튼이 동작하지 않아요',
      answer:
        '리소스팩 미적용, 버전 불일치, 다른 GUI 플러그인 충돌을 확인하세요. ' +
        '최신 JAR와 서버 로그를 함께 제보해 주시면 빠르게 확인할 수 있습니다.',
    },
  ],
};
